...
float* cross_by(float* arr, int N, int len,
                float bin_size, float max_sep) {
    /*
    arr is [f] + [x] + [y], each of them with length N.
    return [bin_inds] + [bin_stat].
    */
    ...
    for (int p1 = 0; p1 < N; p1++) {
        for (int p2 = 0; p2 < N; p2++) {
            d = sqrt((arr[p1+2*N] - arr[p2+2*N]) * (arr[p1+2*N] - arr[p2+2*N]) +
                     (arr[p1+3*N] - arr[p2+3*N]) * (arr[p1+3*N] - arr[p2+3*N]));
            int idx;
            if (d < max_sep) {  // For acceleration.
                if (d <= bin_size && d > 1e-6) {idx = 1;}
                else {idx = (int)(d / bin_size);}
                res[idx+len] = ((arr[p1] * arr[p2+N] + res[idx+len] * res[idx]) /
                                (res[idx] + 1));
                res[idx] += 1;
            }
        }
